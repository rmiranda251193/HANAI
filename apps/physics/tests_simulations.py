...
class PhysicsSimulationModelTests(TestCase):
    ...
    def test_seed_simulations_is_idempotent(self):
        ...
        # a fifteenth (The Ideal Gas Law) and a sixteenth (The Doppler
        # Effect); the seed command still creates every row exactly once
        # and only updates on every re-run.
        self.assertIn("0 created, 16 updated", out.getvalue())
